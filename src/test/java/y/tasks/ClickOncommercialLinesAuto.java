...
public class ClickOncommercialLinesAuto implements Task {
...
//    }
    @Override
    @Step("{0} Click On commercial Lines Auto")
    public <T extends Actor> void performAs(T actor) {
        InsuranceSolutions.COMMERCIAL_LINES_AUTO.resolveFor(actor).waitUntilVisible();
        actor.attemptsTo(

                Click.on(InsuranceSolutions.COMMERCIAL_LINES_AUTO)
        );
    }
