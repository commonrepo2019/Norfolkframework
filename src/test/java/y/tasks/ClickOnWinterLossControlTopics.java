...
public class ClickOnWinterLossControlTopics implements Task {
...
//    }
    @Override
    @Step("{0} Click On Winter Loss Control Topics")
    public <T extends Actor> void performAs(T actor) {
        SafetyTips.WINTER_LOSS_TOPICS.resolveFor(actor).waitUntilVisible();
        actor.attemptsTo(

                Click.on(SafetyTips.WINTER_LOSS_TOPICS)
        );
    }
