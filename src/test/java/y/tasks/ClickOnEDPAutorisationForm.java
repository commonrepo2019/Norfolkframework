...
public class ClickOnEDPAutorisationForm implements Task {
...
//    }
    @Override
    @Step("{0} Click On EDP Autorisation Form")
    public <T extends Actor> void performAs(T actor) {
        BillingAndPayment.EDP_AUTHORISATION_FORM.resolveFor(actor).waitUntilVisible();
        actor.attemptsTo(

                Click.on(BillingAndPayment.EDP_AUTHORISATION_FORM)
        );
    }
