...
public class ClickOncommercialLines implements Task {
...
//    }
    @Override
    @Step("{0} Click On commercial Lines")
    public <T extends Actor> void performAs(T actor) {
        InsuranceSolutions.COMMERCIAL_LINES.resolveFor(actor).waitUntilVisible();
        actor.attemptsTo(

                Click.on(InsuranceSolutions.COMMERCIAL_LINES)
        );
    }
