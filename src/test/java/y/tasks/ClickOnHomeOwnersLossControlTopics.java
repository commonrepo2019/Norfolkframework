...
public class ClickOnHomeOwnersLossControlTopics implements Task {
...
//    }
    @Override
    @Step("{0} Click On Home Owners Loss Control Topics")
    public <T extends Actor> void performAs(T actor) {
        SafetyTips.HOME_OWNER_LOSS_TOPICS.resolveFor(actor).waitUntilVisible();
        actor.attemptsTo(

                Click.on(SafetyTips.HOME_OWNER_LOSS_TOPICS)
        );
    }
